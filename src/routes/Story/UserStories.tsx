          ...
          <section className="mb-12">
            <div className="flex items-baseline justify-between mb-5">
              <div className="flex items-baseline gap-3">
                ...
                <span className="font-ui text-xs text-ns-ink-muted">
                  {recentlyRead.length} {recentlyRead.length === 1 ? "story" : "stories"} in progress
                </span>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {recentlyRead.map((item) => {
                const progressPct = item.totalChapters > 1
                  ? Math.round(((item.chapterIndex) / (item.totalChapters - 1)) * 100)
                  : 100;
                return (
                  <div
                    key={item.storyId}
